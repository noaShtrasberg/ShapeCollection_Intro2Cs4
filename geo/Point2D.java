...
public class Point2D{
	...
	public void scale(Point2D cen, double ratio) {
		...
		Point2D moveVec = new Point2D(cen.x()-(vector.x()*ratio), cen.y()-(vector.y()*ratio));
		this._x = moveVec.x();
		this._y = moveVec.y();
	}
	public void rotate(Point2D cen, double angleDegrees) {
		//On build the function i used this link 
		//https://en.wikipedia.org/wiki/Rotation_matrix
		double angleR = Math.toRadians(angleDegrees);
		double x  = Math.cos(angleR)*(_x - cen.x()) - Math.sin(angleR)*(_y-cen.y()) + cen.x();
		double y = Math.sin(angleR)*(_x - cen.x()) + Math.cos(angleR)*(_y-cen.y()) + cen.y();
		this._x = x;
		this._y = y;
	}
}
